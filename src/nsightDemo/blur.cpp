...
public:
    ...
    ReluLayerGPU(Buffer<float> input, string scheduler)
        ...
        producer(n, c, x, y) = (input(n, c, x, y) + input(n, c, x+1, y) + input(n, c, x+2, y)) / 3; // Brighten up the image
        consumer(n, c, x, y) = (producer(n, c, x, y) + producer(n, c, x, y+1) + producer(n, c, x, y+2)) / 3; // Blur
    ...
    bool schedule_for_gpu() {
        ...
        if (scheduler.empty()) {
            if (target.has_feature(Target::CUDA)) {
                ...
                //    .vectorize(y_inner, 8);*/
                ////producer.store_at();
                //producer.compute_at(consumer, x_outer).gpu_threads(x ,y);

                consumer.fuse(n, c, nc)
                    .gpu_tile(nc, x, nco, x_outer, nci, x_inner, 8, 8)
                    //.gpu_lanes(x_inner)
                    .vectorize(y, 8);
                producer.compute_at(consumer, nci);
            }
            else {
...
int main(int argc, char** argv) {
    ...
    const int batch_size = 8, width = 1026, height = 1026, channels_in = 3;
    string scheduler = "";
    ...
    ReluLayerGPU blur_layer(input, scheduler);
